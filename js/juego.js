...
var instrucciones = ["Utilizar las flechas del teclado para mover las piezas",
"Mover las piezas hasta alcanzar la imagen del objetivo"];
...
var ganadora= [//esta es la grilla ganadora
  [1, 2, 3],
  [4, 5, 6],
  [7, 8, 9]
];
...
function mostrarCartelGanador() {
  if (chequearSiGano){
      alert("Ganaste!!!! Empecemos de nuevo");
    mezclarPiezas(30);}
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
    let posicion_temp= grilla[filaPos1][columnaPos1];
    grilla[filaPos1][columnaPos1]= grilla[filaPos2][columnaPos2];
    grilla[filaPos2][columnaPos2]= posicion_temp;
}

// Actualiza la posición de la pieza vacía
function actualizarPosicionVacia(nuevaFilaPiezaVacia, nuevaColumnaPiezaVacia) {
  filaVacia = nuevaFilaPiezaVacia;
  columnaVacia = nuevaColumnaPiezaVacia;
}


// Para chequear si la posicón está dentro de la grilla.
function posicionValida(fila, columna) {
  if((fila < 0) || (fila > 2) || (columna < 0) || (columna > 2)){//si no esta dentro de los indices de la grilla retorna falso
    return false;
  }
    else
    {
